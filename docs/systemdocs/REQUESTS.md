...
| `request_stepstone` | Spends a Stepstone to stand in any Location that character KNOWS — stood in or seen from a doorway (`MAP.md` §3b). No ⬢, no Move, no adjacency, no Action. Escorts are left behind and no gate line is posted | — | Restores the bottle and the old `locationId`/`zoneId` |
