...
(`craftMoveCost`'s `share` case), and a `turnsCost: 1` cure (always a Gambit
on the ladder, tier 7) is the whole thing. A Gambit heal never reaches the
pool question at all: it files a `MOVE`/`GAMBIT` Action, and
