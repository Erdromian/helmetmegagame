...
test("faces 2 to 4 have no column, so nothing is ever drawn on one", () => {
  for (const roll of [2, 3, 4]) {
    assert.equal(columnFor(roll), null);
    assert.equal(drawFromPool([tag("common")], roll), null);
  }
});
