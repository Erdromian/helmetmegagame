...
async function notableWatermarks(client, places, character) {
  ...
  try {
    const floor = await feedWipeFloor(client);
    const base = { deletedAt: null, seq: seqFilterAbove(floor), NOT: { characterId: character.id } };

    const [convRows, mentionRows] = await Promise.all([
      convKeys.length
        ? client.archiveEntry.groupBy({
            by: ["placeKey"],
            where: { ...base, placeKey: { in: convKeys } },
            _max: { seq: true },
          })
        : [],
      client.archiveEntry.groupBy({
        by: ["placeKey"],
        where: { ...base, placeKey: { in: allKeys }, content: { contains: `{char:${character.id}}` } },
        _max: { seq: true },
      }),
    ]);
